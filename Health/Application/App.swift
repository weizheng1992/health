...
let JMessageAppKey = "137efa3e68baadcd07b7cf80"
...
public final class App {
    ...
    var user: User? = Preferences().get(for: PreferencesKey.user) {
        didSet {
            Preferences().set(user, for: PreferencesKey.user)
        }
    }
    
}

func JKString(_ key: String) -> String {
    return NSLocalizedString(key, comment: "")
}

extension PreferencesKey {
    static let version = Preferences.Key<String>("version")
    static let user = Preferences.Key<User>("user")
}

extension UIColor {
    static let main = UIColor(red: 17 / 255.0, green: 182 / 255.0, blue: 162 / 255.0, alpha: 1.0)
}




func isValidePhoneNumber(_ phoneNumber: String) -> Bool {
    if phoneNumber.count == 0 {
        return false
    }
    
    let mobile = "^(13[0-9]|14[0-9]|15[0-9]|18[0-9]|17[0-9]|147)\\d{8}$"
...
extension UIViewController {
    ...
    fileprivate struct AssociatedKeys {
        ...
        static var navBarTintColor: UIColor = .main
    }
